...
export function validateModelParameters(params: ModelParameters): void {
  if (params.temperature !== undefined) {
    if (
      params.temperature < VALIDATION_LIMITS.MIN_TEMPERATURE ||
  ...
  if (params.top_p !== undefined) {
    if (params.top_p < VALIDATION_LIMITS.MIN_TOP_P || params.top_p > VALIDATION_LIMITS.MAX_TOP_P) {
      throw new Error(`top_p must be between ${VALIDATION_LIMITS.MIN_TOP_P} and ${VALIDATION_LIMITS.MAX_TOP_P}`);
    }
  }

  if (params.frequency_penalty !== undefined) {
    if (
      params.frequency_penalty < VALIDATION_LIMITS.MIN_FREQUENCY_PENALTY ||
  ...
  if (params.presence_penalty !== undefined) {
    if (
      params.presence_penalty < VALIDATION_LIMITS.MIN_PRESENCE_PENALTY ||
  ...
  if (params.max_tokens !== undefined) {
    if (params.max_tokens < VALIDATION_LIMITS.MIN_MAX_TOKENS || params.max_tokens > VALIDATION_LIMITS.MAX_MAX_TOKENS) {
      throw new Error(
